...
int atmel_ecc_create_pms(int slotId, const uint8_t* peerKey, uint8_t* pms)
{
    ...
    ret = atcab_ecdh_base(ECDH_MODE_SOURCE_TEMPKEY | ECDH_MODE_COPY_OUTPUT_BUFFER, slotId, peerKey, pms, NULL);
#endif
        ...
        int side, void* ctx)
{
    ...
    if (otherKey->dp->id == ECC_SECP256R1) {
        ...
        if (side == WOLFSSL_CLIENT_END)
        {
            ...
            int slotId = GENKEY_PRIVATE_TO_TEMPKEY;
#endif    
            if (slotId == ATECC_INVALID_SLOT)
                return WC_HW_WAIT_E;
            tmpKey.slot = slotId;
