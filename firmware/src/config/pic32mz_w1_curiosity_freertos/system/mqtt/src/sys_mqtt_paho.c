...
void SYS_MQTT_Paho_Task(SYS_MODULE_OBJ obj)
{
    ...
    case SYS_MQTT_STATUS_SOCK_CONNECTED:
    {
        ...
        /* Call the Application CB to create 'Connected' event */
        if (hdl->callback_fn)
        {
            hdl->callback_fn(SYS_MQTT_EVENT_MSG_SOCK_CONNECTED,
                             (void*)(&hdl->sCfgInfo.sBrokerConfig),
                             0,
                             hdl->vCookie);
        }
    ...
    case SYS_MQTT_STATUS_WAIT_FOR_MQTT_CONACK:
    {
        ...
        if (rc == SUCCESS)
        {
            ...
            {
                SYS_MQTTDEBUG_DBG_PRINT(g_AppDebugHdl, MQTT_DATA, "Subscribing to Topic = %d, %s\r\n", hdl->sCfgInfo.subscribeCount, hdl->sCfgInfo.sSubscribeConfig[0].topicName);
    ...
    {//SYS_CONSOLE_MESSAGE("SYS_MQTT_STATUS_MQTT_CONNECTED:\r\n");
