...
<img src="images/Environment1.png" width=700 style="display: block; margin: auto; float: center"><br>

The FreeRTOS&trade; based application will host a TCP client in Wi-Fi STA mode connecting to the Home-AP. To ensure proper Wi-Fi connectivity provide the necessary details, see [here](#firmware).<br> By default, every time the IoT device is powered-up, a TCP client socket connection to Kudelski server will be established to either initially set-up, confirm or renew its credentials. Only if the so called in-field provisioning process has been finished successfully, the device is able to establish a secure connection to any cloud-computing provider deploying PKI, e.g. Amazon Web Services.<br> <b>Note:</b> At Kudelski account: Select "My Devices" and double-check that the device state is set to "Onboarded". At AWS account: Select Iot Core -> Security -> Certificates and double-check that a device related certificate ID has been created and activated. <br> 
<b>TODO: Check who and when is the certificate ID at AWS generated???</b><br>
...
Just prior the MQTT based demo is started, the device needs to be registered once to AWS. This step is hidden to the user and is automatically done during first cloud access.<br>
<b>TODO: Check when is this happening exactly???</b><br>
<b>Note:</b> At AWS account: Select Iot Core -> All devices -> Things and double-check that the device has been registered.
Once a successful connection to the MQTT broker has been establisch, the application is subscribing to a specific MQTT-topic, followed by creating a default device shadow. After that, an internal timer will be started to initiate the LED toggle mechanism. Everytime an LED state change is taking place a reporting message will be published to the broker and a related change notification must be received from the cloud.
...
- For Wi-Fi connectivity insert the credentials either by means of selecting the <i>WIFI SERVICE</i> in the MCC Project Graph <b>or</b> in <i>configuration.h</i>. Make sure the router is connected to the internet.<br>
